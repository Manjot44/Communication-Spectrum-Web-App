...
function CreateDailySchedulesScratch({ token, setTokenFunc }) {
  ...
  const [visualSupports, setVisualSupports] = useState({
    Monday: { steps: [], stepImages: [], stepNames: [], stepTimes: [], image: null },
    Tuesday: { steps: [], stepImages: [], stepNames: [], stepTimes: [], image: null },
    Wednesday: { steps: [], stepImages: [], stepNames: [], stepTimes: [], image: null },
    Thursday: { steps: [], stepImages: [], stepNames: [], stepTimes: [], image: null },
    Friday: { steps: [], stepImages: [], stepNames: [], stepTimes: [], image: null },
    Saturday: { steps: [], stepImages: [], stepNames: [], stepTimes: [], image: null },
    Sunday: { steps: [], stepImages: [], stepNames: [], stepTimes: [], image: null },
  });
  ...
  const removeStep = (index, id) => {
    setVisualSupports((prevSupports) => ({
      ...
      [selectedDay]: {
        ...
        stepImages: prevSupports[selectedDay].stepImages.filter((_, imgIndex) => imgIndex !== index),
        stepNames: prevSupports[selectedDay].stepNames.filter((_, imgIndex) => imgIndex !== index),
        stepTimes: prevSupports[selectedDay].stepTimes.filter((_, imgIndex) => imgIndex !== index),
      },
    }));
  };
                ...
                <Typography variant="h5" component="div" style={{ fontFamily: 'Poppins' }}>
                  ...
                  <Grid container spacing={2} style={{ padding: '2%' }}>
                    <Grid item xs={12} md={6}>
                      <VisualSupportImage image={visualSupports[selectedDay].image} setImage={setImage} uniqueID={-1} imgHeight={"55vh"} deleteImage={() => setImage(null)}/>
                    </Grid>
                      ...
                      {visualSupports[selectedDay].steps.map((step, index) => (
                        isHorizontal ? (
                          <TaskAnalysesStepHorizontal
                            key={step.id}
                            index={index}
                            image={visualSupports[selectedDay].stepImages[index]}
                            ...
                            stepName={visualSupports[selectedDay].stepNames[index]}
                            setTime={(newTime) => updateStepTime(index, newTime)}
                            stepTime={visualSupports[selectedDay].stepTimes[index]}
                          />
                        ) : (
                          <TaskAnalysesStep
                            key={step.id}
...
export default CreateDailySchedulesScratch;